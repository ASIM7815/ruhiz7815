-- Safe SQL to create tables (skips if already exists)
...
BEGIN
    ...
    -- Add file_assets foreign key
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'file_assets_user_id_fkey'
    ) THEN
        ALTER TABLE "file_assets" 
        ADD CONSTRAINT "file_assets_user_id_fkey" 
        FOREIGN KEY ("user_id") 
        REFERENCES "users"("id") 
        ON DELETE RESTRICT ON UPDATE CASCADE;
    END IF;
