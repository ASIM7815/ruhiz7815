...
type ViewerStatus = "NONE" | "PENDING" | "MEMBER" | "LEADER";
...
export default function StudyGroupsPage() {
  ...
  // Load browse groups (includes viewerStatus when logged in)
  ...
  // Load user’s own groups (leader + member)
  const loadMyGroups = useCallback(async (showSpinner = true) => {
    if (!userId) return;
    if (showSpinner) setMyLoading(true);
    try {
      const res = await fetch("/api/study-groups?member=me");
      if (!res.ok) return;
      const data = await res.json();
  ...
  // Poll for new join requests when on "mine" tab
  useEffect(() => {
    if (!userId) return;
    const intervalId = window.setInterval(() => {
      if (document.visibilityState === "visible" && tab === "mine") {
        loadMyGroups(false);
      }
    }, 8000);
    return () => window.clearInterval(intervalId);
  }, [userId, tab, loadMyGroups]);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await loadGroups();
        if (tab === "mine") await loadMyGroups(false);
      } else {
  ...
  async function handleJoinRequest(groupId: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setGroups((prev) =>
          prev.map((g) => (g.id === groupId ? { ...g, viewerStatus: "PENDING" } : g))
        );
      } else {
        const err = await res.json().catch(() => ({}));
        if (err.error?.includes("Already a member") || err.error?.includes("leader")) {
          setGroups((prev) =>
            prev.map((g) => (g.id === groupId ? { ...g, viewerStatus: "MEMBER" } : g))
          );
        } else {
  ...
  async function handleRequest(
    ...
    try {
      const res = await fetch(`/api/study-groups/${groupId}/join/${requestId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        toast.error(err.error || "Could not process request.");
        return;
      }
      setMyGroups((prev) =>
        prev.map((g) =>
          g.id === groupId
            ? {
                ...g,
                memberCount: action === "ACCEPTED" ? g.memberCount + 1 : g.memberCount,
  ...
  const totalPending = myGroups
    .filter((g) => g.userRole === "LEADER")
    .reduce((s, g) => s + g.pendingRequests.length, 0);

  function JoinButton({ group }: { group: StudyGroup }) {
      ...
      {tab === "browse" ? (
        ...
        myLoading ? (
          ...
          <div className="space-y-4">
            {/* Leader groups first, then member groups */}
            {[...myGroups]
              .sort((a, b) => (a.userRole === "LEADER" ? -1 : b.userRole === "LEADER" ? 1 : 0))
              .map((group) => (
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <Badge
                          variant={group.userRole === "LEADER" ? "default" : "outline"}
                          className="text-xs"
                        >
                          {group.userRole === "LEADER" ? "Admin" : "Member"}
                        </Badge>
                      </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <Button
              ...
              className="w-full"
              disabled={creating || !form.name.trim() || !form.subject.trim()}
            >
              {creating ? (
